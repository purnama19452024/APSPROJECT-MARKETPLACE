...
<section class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <h1 class="text-2xl font-bold text-gray-800 mb-6"><i class="fas fa-edit text-orange-500 mr-2"></i>Ubah Alamat</h1>

    <form action="{{ route('addresses.update', $address) }}" method="POST" class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="label" class="w-full border rounded-lg p-2.5 text-sm @error('label') border-red-400 @enderror" required>
                    ...
                    <option value="Kost" {{ old('label', $address->label) == 'Kost' ? 'selected' : '' }}>Kost</option>
            ...
            <div>
                ...
                <input type="text" name="recipient_name" value="{{ old('recipient_name', $address->recipient_name) }}" class="w-full border rounded-lg p-2.5 text-sm @error('recipient_name') border-red-400 @enderror" required>
            ...
            <div>
                ...
                <input type="text" name="recipient_phone" value="{{ old('recipient_phone', $address->recipient_phone) }}" class="w-full border rounded-lg p-2.5 text-sm @error('recipient_phone') border-red-400 @enderror">
                @error('recipient_phone')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Kode Pos</label>
                <input type="text" name="postal_code" value="{{ old('postal_code', $address->postal_code) }}" class="w-full border rounded-lg p-2.5 text-sm @error('postal_code') border-red-400 @enderror">
        ...
        <div class="mb-4">
            ...
            <select name="province_code" id="province" class="w-full border rounded-lg p-2.5 text-sm @error('province_code') border-red-400 @enderror" onchange="loadCities(this.value)">
                ...
                @foreach($provinces as $p)
                    <option value="{{ $p->code }}" {{ old('province_code', $address->province_code) == $p->code ? 'selected' : '' }}>{{ $p->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="address" rows="3" class="w-full border rounded-lg p-2.5 text-sm @error('address') border-red-400 @enderror" required placeholder="Contoh: Jl. Merdeka No. 123, RT 01 RW 02">{{ old('address', $address->address) }}</textarea>
            @error('address')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
        </div>

        <div class="mb-6">
            <label class="flex items-center gap-2">
                <input type="checkbox" name="is_default" value="1" {{ old('is_default', $address->is_default) ? 'checked' : '' }} class="rounded border-gray-300 text-orange-500 focus:ring-orange-500">
                <span class="text-sm text-gray-700">Jadikan alamat utama</span>
            </label>
        </div>

        <button type="submit" class="w-full bg-orange-500 text-white py-3 rounded-xl font-semibold hover:bg-orange-600 transition"><i class="fas fa-save mr-2"></i>Simpan Perubahan</button>
    </form>
</section>
@endsection
...
@push('scripts')
<script>
    var cityLoaded = '{{ old("city_code", $address->city_code) }}';
    var districtLoaded = '{{ old("district_code", $address->district_code) }}';

...
<body class="font-sans antialiased bg-gray-100">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-green-500">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center"><i class="fas fa-box text-green-600 text-xl"></i></div>
                        </div>
                    </div>
                    <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-orange-500">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="w-12 h-12 bg-orange-100 rounded-lg flex items-center justify-center"><i class="fas fa-receipt text-orange-600 text-xl"></i></div>
                        </div>
                    </div>
                    <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-purple-500">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center"><i class="fas fa-money-bill-wave text-purple-600 text-xl"></i></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-3">
                            @foreach($recentTransactions as $t)
                                <div class="flex items-center justify-between text-sm">
                                    <div>
                                        <p class="font-medium">{{ $t->invoice }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6">
                        <h3 class="font-bold text-gray-800 mb-4">User Terbaru</h3>
                        <div class="space-y-3">
                            @foreach($recentUsers as $u)
                                <div class="flex items-center space-x-3 text-sm">
                                    <img src="{{ $u->photo_url }}" alt="" class="w-8 h-8 rounded-full object-cover">
                                    <div>
                    ...
                    @if($flashSaleProducts->count() > 0)
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                            @foreach($flashSaleProducts as $fp)
                                <a href="{{ route('admin.products.edit', $fp) }}" class="border rounded-lg p-2 hover:shadow-md transition text-center group">
                                    @if($fp->primaryImage)
                                    ...
                                    @endif
                                    <p class="text-xs font-medium text-gray-800 truncate group-hover:text-orange-500">{{ $fp->name }}</p>
    ...
    <script>
        ...
        new Chart(ctx, {
            type: 'line',
            data: {
                labels: [@foreach($salesChart as $s) '{{ $s->date }}', @endforeach],
                datasets: [{
                    label: 'Penjualan',
                    data: [@foreach($salesChart as $s) {{ $s->total }}, @endforeach],
                    borderColor: '#f97316',

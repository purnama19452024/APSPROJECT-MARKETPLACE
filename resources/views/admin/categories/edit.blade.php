        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                <div class="max-w-lg mx-auto bg-white rounded-xl shadow-sm p-6">
                    <h1 class="text-xl font-bold text-gray-800 mb-4">Edit Kategori</h1>
                    <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
                        @csrf @method('PUT')
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Nama Kategori</label>
                                <input type="text" name="name" value="{{ $category->name }}" class="w-full border rounded-lg p-2 text-sm" required>
                            </div>
                            <div>
                                ...
                                @if($category->icon)
                                    <img src="{{ $category->icon }}" alt="" class="w-16 h-16 mt-2 rounded-lg object-cover">
                                @endif
                            </div>
                            <div>

    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <h1 class="text-2xl font-bold text-gray-800 mb-2">Ulasan</h1>
                <p class="text-sm text-gray-500 mb-6">Kelola ulasan produk dari pelanggan</p>
                                ...
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="p-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            @endif
                                            <span class="font-medium text-xs truncate max-w-[150px]">{{ $r->product->name }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-3 text-gray-600 max-w-[250px] truncate">{{ $r->review ?? '-' }}</td>
                                    <td class="p-3 text-gray-500 text-xs">{{ $r->created_at->format('d M Y') }}</td>
                                    <td class="p-3 text-center">
                                        <form action="{{ route('admin.reviews.destroy', $r) }}" method="POST" onsubmit="return confirm('Hapus ulasan ini?')">
                                            @csrf @method('DELETE')
